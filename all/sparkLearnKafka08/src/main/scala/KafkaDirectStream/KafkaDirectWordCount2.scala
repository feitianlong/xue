...
object KafkaDirectkafkatopic2 {
  def main(args: Array[String]): Unit = {
    ...
    kafkaStream.foreachRDD( kafkardd => {
      // 读取偏移量,并存放到offsetRanges
      offsetRanges = kafkardd.asInstanceOf[HasOffsetRanges].offsetRanges
      // 从kafka读取的消息,即获取value
      val linesrdd: RDD[String] = kafkardd.map(_._2)
      //对RDD进行操作，触发Action
      linesrdd.foreachPartition(partition =>
        partition.foreach(x => {
          println(x)
        })
      )
      for (o <- offsetRanges) {
        ...
        //  /g001/offsets/kafkatopic/0/20000
